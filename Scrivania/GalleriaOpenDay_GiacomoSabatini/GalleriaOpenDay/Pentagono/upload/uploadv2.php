...
function addNodeInfoXml($xml,$user,$data,$ora,$peso,$sfile){
...
if(!isLogged())   {//se non è loggato torna al form e lo script termina
	...
	if (file_exists("../upload_pde/" . $_FILES["file"]["name"])) {
		...
		addNodeInfoXml($xml,$user,$data,$ora,$peso,$_FILES["file"]["name"]);
		echo($xml);
		$xml->asXML($strfile);
	}
